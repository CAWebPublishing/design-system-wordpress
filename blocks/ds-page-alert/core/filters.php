...
add_filter( 'script_loader_tag', 'cagov_design_system_ds_page_alert_script_loader_tag', 10, 3 );
...
if( ! function_exists('cagov_design_system_block_categories') ){
	/**
	* Register ds-page-alert Gutenberg Block categories to the Block editor
	*
...
if( ! function_exists('cagov_design_system_allowed_block_types') ){
	/**
	* ds-page-alert Allowed Block Types
	*
	* Removes all blocks or patterns from Gutenberg and returns ds-page-alert Blocks.
	*
	...
	*/
	function cagov_design_system_allowed_block_types( $allowed_blocks ) {
		...
		// Dynamically get a list of the ds-page-alert blocks.
...
if( ! function_exists('cagov_design_system_ds_page_alert_script_loader_tag') ){
	/**
	...
	* @return string
	*/
	function cagov_design_system_ds_page_alert_script_loader_tag( $tag, $handle, $src ) {
		// Register script as module.
		if ( 'cagov_design_system-ds-page-alert' === $handle ) {
			$tag = sprintf('<script type="module" id="cagov_design_system-ds-page-alert-js" src="%1$s"></script>', $src);
		}
