...
add_filter( 'script_loader_tag', 'caweb_link_grid_card_script_loader_tag', 10, 3 );
...
if( ! function_exists('caweb_link_grid_card_script_loader_tag') ){
	/**
	...
	* @return string
	*/
	function caweb_link_grid_card_script_loader_tag( $tag, $handle, $src ) {
		// Register script as module.
		if ( 'caweb-link-grid-card' === $handle ) {
			$tag = sprintf('<script type="module" id="caweb-link-grid-card-js" src="%1$s"></script>', $src);
		}
