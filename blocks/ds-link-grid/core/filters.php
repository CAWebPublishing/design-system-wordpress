...
add_filter( 'script_loader_tag', 'cagov_design_system_ds_link_grid_script_loader_tag', 10, 3 );
...
if( ! function_exists('cagov_design_system_block_categories') ){
	/**
	* Register ds-link-grid Gutenberg Block categories to the Block editor
	*
...
if( ! function_exists('cagov_design_system_allowed_block_types') ){
	/**
	* ds-link-grid Allowed Block Types
	*
	* Removes all blocks or patterns from Gutenberg and returns ds-link-grid Blocks.
	*
	...
	*/
	function cagov_design_system_allowed_block_types( $allowed_blocks ) {
		...
		// Dynamically get a list of the ds-link-grid blocks.
...
if( ! function_exists('cagov_design_system_ds_link_grid_script_loader_tag') ){
	/**
	...
	* @return string
	*/
	function cagov_design_system_ds_link_grid_script_loader_tag( $tag, $handle, $src ) {
		// Register script as module.
		if ( 'cagov_design_system-ds-link-grid' === $handle ) {
			$tag = sprintf('<script type="module" id="cagov_design_system-ds-link-grid-js" src="%1$s"></script>', $src);
		}
