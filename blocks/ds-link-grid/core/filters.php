...
add_filter( 'script_loader_tag', 'caweb_ds_link_grid_script_loader_tag', 10, 3 );
...
if( ! function_exists('caweb_ds_link_grid_script_loader_tag') ){
	/**
	...
	* @return string
	*/
	function caweb_ds_link_grid_script_loader_tag( $tag, $handle, $src ) {
		// Register script as module.
		if ( 'caweb-ds-link-grid' === $handle ) {
			$tag = sprintf('<script type="module" id="caweb-ds-link-grid-js" src="%1$s"></script>', $src);
		}
