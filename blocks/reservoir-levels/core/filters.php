...
add_filter( 'script_loader_tag', 'caweb_reservoir_levels_script_loader_tag', 10, 3 );
...
if( ! function_exists('caweb_reservoir_levels_script_loader_tag') ){
	/**
	...
	* @return string
	*/
	function caweb_reservoir_levels_script_loader_tag( $tag, $handle, $src ) {
		// Register script as module.
		if ( 'caweb-reservoir-levels' === $handle ) {
			$tag = sprintf('<script type="module" id="caweb-reservoir-levels-js" src="%1$s"></script>', $src);
		}
