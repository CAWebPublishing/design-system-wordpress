...
add_filter( 'script_loader_tag', 'cagov_design_system_reservoir_levels_script_loader_tag', 10, 3 );
...
if( ! function_exists('cagov_design_system_block_categories') ){
	/**
	* Register reservoir-levels Gutenberg Block categories to the Block editor
	*
...
if( ! function_exists('cagov_design_system_allowed_block_types') ){
	/**
	* reservoir-levels Allowed Block Types
	*
	* Removes all blocks or patterns from Gutenberg and returns reservoir-levels Blocks.
	*
	...
	*/
	function cagov_design_system_allowed_block_types( $allowed_blocks ) {
		...
		// Dynamically get a list of the reservoir-levels blocks.
...
if( ! function_exists('cagov_design_system_reservoir_levels_script_loader_tag') ){
	/**
	...
	* @return string
	*/
	function cagov_design_system_reservoir_levels_script_loader_tag( $tag, $handle, $src ) {
		// Register script as module.
		if ( 'cagov_design_system-reservoir-levels' === $handle ) {
			$tag = sprintf('<script type="module" id="cagov_design_system-reservoir-levels-js" src="%1$s"></script>', $src);
		}
