...
if ( ! function_exists( 'cagov_design_system_card_get_min_file' ) ) {
	/**
	 ...
	 * @return string
	 */
	function cagov_design_system_card_get_min_file( $f, $ext = 'css' ) {
		...
		if ( ! CAGOV_DESIGN_SYSTEM_DEBUG && file_exists( __DIR__ . str_replace( ".$ext", ".min.$ext", $f ) ) ) {
			return Card_URI . str_replace( ".$ext", ".min.$ext", $f );
		} else {
			return Card_URI . $f;
		}
	}
