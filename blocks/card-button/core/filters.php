...
add_filter( 'script_loader_tag', 'cagov_design_system_card_button_script_loader_tag', 10, 3 );

if ( ! function_exists( 'cagov_design_system_block_categories' ) ) {
	/**
	 * Register Card Button Gutenberg Block categories to the Block editor
	 *
...
if ( ! function_exists( 'cagov_design_system_card_button_script_loader_tag' ) ) {
	/**
	 ...
	 * @return string
	 */
	function cagov_design_system_card_button_script_loader_tag( $tag, $handle, $src ) {
		// Register script as module.
		if ( 'cagov-design-system-card-button' === $handle ) {
			$tag = sprintf( '<script type="module" id="cagov-design-system-card-button-js" src="%1$s"></script>', $src );
		}
