...
add_filter( 'script_loader_tag', 'caweb_drought_map_script_loader_tag', 10, 3 );
...
if( ! function_exists('caweb_drought_map_script_loader_tag') ){
	/**
	...
	* @return string
	*/
	function caweb_drought_map_script_loader_tag( $tag, $handle, $src ) {
		// Register script as module.
		if ( 'caweb-drought-map' === $handle ) {
			$tag = sprintf('<script type="module" id="caweb-drought-map-js" src="%1$s"></script>', $src);
		}
