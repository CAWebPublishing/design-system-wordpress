...
add_filter( 'script_loader_tag', 'cagov_design_system_link_grid_card_script_loader_tag', 10, 3 );
...
if ( ! function_exists( 'cagov_design_system_block_categories' ) ) {
	/**
	 * Register Link Grid Card Gutenberg Block categories to the Block editor
	 *
...
if ( ! function_exists( 'cagov_design_system_allowed_block_types' ) ) {
	/**
	 * Link Grid Card Allowed Block Types
	 *
	 * Removes all blocks or patterns from Gutenberg and returns Link Grid Card Blocks.
	 *
...
if ( ! function_exists( 'cagov_design_system_link_grid_card_script_loader_tag' ) ) {
	/**
	 ...
	 * @return string
	 */
	function cagov_design_system_link_grid_card_script_loader_tag( $tag, $handle, $src ) {
		// Register script as module.
		if ( 'cagov-design-system-link-grid-card' === $handle ) {
			$tag = sprintf( '<script type="module" id="cagov-design-system-link-grid-card-js" src="%1$s"></script>', $src );
		}
